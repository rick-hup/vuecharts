...
│   │   ├── area-charts/    # gradient (SVG <defs><linearGradient> + fill="url(#id)"), legend (multi-series+<Legend> :fill-opacity="0.3"), stacked (stack-id="a" solid fill), step (type="step")
│   │   ├── bar-charts/     # horizontal (layout="vertical" + XAxis type="number" + YAxis type="category"), label (<LabelList> child slot), negative (<Cell> conditional fill)
│   │   ├── composed-charts/# simple (<Area>+<Bar :bar-size>+<Line> on one ComposedChart)
│   │   ├── line-charts/    # simple (type="monotone"), dots (:dot={fill,r} :active-dot={r}), dashed (stroke-dasharray per Line), multi (two Lines), step (type="step")
│   │   ├── pie-charts/     # donut (inner-radius+outer-radius), half (start-angle=180 end-angle=0), label (:label="true"); all use <Cell v-for>
...
│   ├── pages/              # index.vue (home) — two full-screen scroll-snap sections (`:global(html) scroll-snap-type: y mandatory`, each section `scroll-snap-align: start`): (1) `.landing-hero` (height: 100dvh — full viewport) centered hero: dot-pattern bg + two floating glow orbs (`.hero-glow-orange` x:23s/y:19s drift, `.hero-glow-teal` x:17s/y:29s drift — prime-number durations for organic movement); staggered hero content via `containerVariants` (`staggerChildren:0.1, delayChildren:0.15`) + `itemVariants` (hidden: `opacity:0, y:40, blur(4px)` → visible: `opacity:1, y:0, blur(0px)`, spring `stiffness:120 damping:20`); `motion.div` wraps all hero content with `initial="hidden" animate="visible"`; install snippet `.landing-install` pill (click-to-copy `npm install vccs`, UIcon toggles `i-lucide-copy`↔`i-lucide-check` via `installCopied` ref), title `vccs v0.1`, tagline, CTA buttons (Get Started + GitHub via UButton), feature pills (Vue 3/TypeScript/Composable/Animated), scroll indicator `.landing-scroll` bounces via `motion.div` `animate: { y: [0,8,0] }` repeat Infinity easeInOut, calls `scrollToShowcase()` (`getElementById('showcase')?.scrollIntoView({ behavior: 'smooth' })`); (2) `.showcase` (height: 100dvh-64px) evilcharts-style: left 40% `.showcase-content` z-10 (heading "Explore Charts" + desc + category tabs chevron `>`) + right 55% `.showcase-charts` absolute overflow-hidden (left fade gradient 150px + background chart grid + featured card); `ChartEntry` interface: `{ component, title, desc, category, trend, trendUp }`; `allCharts` 28 entries distributed round-robin into 4 cols (`cols[i % 4]`), cols 1&3 get `chart-col-offset` (margin-top 4.5rem); `motion.div` spring `{ stiffness:75, damping:25 }` animates `y` to `gridAnimateY` (per-category offsets: bar:-850, area:-1200, line:-1600, pie:-2000, radar:-500, radial:-300); `activeCategory` ref drives grid scroll + featured card; `featuredLoaders` map (bar/area/line/pie/radar/radial → import paths) + `featuredMeta` map (per-category title/desc/trend); `featuredComponent = shallowRef<Component | null>` (prevents flicker); `onMounted` eagerly loads bar chart via `featuredLoaders.bar()`; `switchCategory(key)` async — awaits `featuredLoaders[key]?.()` then sets `featuredComponent.value = mod.default` + `activeCategory.value = key`; featured card `.featured-card` 26rem wide, absolutely centered z-10, `overflow:visible`, `::before` frosted glow radial-gradient (ellipse); `<AnimatePresence mode="wait">` wraps `motion.div` keyed by `activeCategory` with `initial/animate/exit: { opacity, scale:0.95→1→0.95 }` `{ duration: 0.3, ease: 'easeOut' }` (no delay); `asyncChart()`: `defineAsyncComponent({ loader, loadingComponent: ChartLoading, delay: 0, errorComponent: ChartError, timeout: 10000 })` (ChartError renders null; ChartLoading renders a 300px-tall div placeholder); `copyInstall()` clipboard with textarea+execCommand fallback; trend badges: `.trend-up` (#dcfce7/#16a34a light, rgba(22,163,74,0.15)/#4ade80 dark) / `.trend-down` (#fee2e2/#ef4444 light, rgba(239,68,68,0.15)/#f87171 dark); responsive: stacks at 768px, showcase-charts `position:relative height:400px`; `prefers-reduced-motion` freezes grid at `translateY(-850px) !important`; uses `useSeoMeta` + UButton/UIcon (Nuxt UI)
