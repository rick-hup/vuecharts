...
- `Line` `dot` prop: plain object `{ stroke, strokeWidth, r }` for custom dot styling; `hide` prop to conditionally show/hide series; `legendType="none"` + `tooltipType="none"` exclude a Line from legend/tooltip (trailing icon pattern); `connectNulls` bridges gaps over null values; `label` prop: plain object `{ fill }` for inline data labels
...
- **LineChart stories** (`cartesian/line/__stories__/LineChart.stories.tsx`, `title: 'examples/LineChart'`): exports `Simple`, `Dashed`, `Vertical`, `BiAxial`, `VerticalWithSpecifiedDomain`, `ConnectNulls`, `WithXAxisPadding`, `LineChartHasMultiSeries`, `LineChartAxisInterval`, `WithBrush`, `HideOnLegendClick`, `LineTrailingIcon`, `ReversedXAxis`, `ChangingDataKey`, `ToggleBetweenDataKeys`; `LineChartAxisInterval` demonstrates all `interval` variants (`preserveEnd`, `preserveStart`, `preserveStartEnd`, `0`); `BiAxial` uses two `YAxis` with `yAxisId`; `LineTrailingIcon` uses a second `Line` with `legendType="none"` `tooltipType="none"` for trailing icon effect
