...
- Usage in `Bar`: `<LabelList {...labelProps} data={barData.value} />` rendered after `BarRectangles`; when `label` prop is an object, its keys are spread as props onto `LabelList`; only rendered when `!isAnimating.value && props.label` — suppressed during animation to prevent label flicker
