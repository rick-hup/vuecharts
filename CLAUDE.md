...
├── app/
│   ├── components/ui/chart/  # ChartContainer.vue (wraps ResponsiveContainer + vccs SVG theme CSS selectors), index.ts
│   └── pages/              # index.vue (card-grid nav), bar-charts.vue (10 Bar variants), area.vue
└── nuxt.config.ts          # compatibilityVersion 4, Tailwind v4 (@tailwindcss/vite), shadcn-nuxt (prefix: '', componentDir: ./app/components/ui)
```

**Playground stack**: Nuxt 3, Tailwind CSS v4, shadcn-nuxt, lucide-vue-next, vccs (`workspace:*`). `ChartContainer` wraps `<ResponsiveContainer>` with vccs SVG theme CSS selectors (same pattern as docs site). Pages: `index.vue` (card-grid nav with shadcn Cards + lucide icons), `bar-charts.vue` (10 variants: basic, horizontal, multiple, stacked+legend, label, custom-label, mixed per-bar fills, active hover, negative values, interactive toggle). Bar chart data: 6-month visitor data (Jan-Jun) + 90-day date series for interactive chart. State: `activeBarIndex` ref for hover highlight, `activeChart` ref for desktop/mobile toggle.
