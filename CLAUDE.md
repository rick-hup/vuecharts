...
Customization uses **named slots**: `shape`, `dot`, `activeDot`, `label`, `content`, `cursor`, `tick`, `horizontal`, `vertical`. Example: `<Bar>{{ shape: (props) => <Custom {...props} /> }}</Bar>`. CartesianGrid custom line renderers use `#horizontal` / `#vertical` slots (Function type was removed from `horizontal`/`vertical` props).
...
- `Tooltip #content slot` (**preferred Vue pattern**): `<Tooltip><template #content="tooltipProps"><MyContent v-bind="tooltipProps" /></template></Tooltip>`; slot takes priority over `content` prop; `tooltipProps` type is `TooltipContentProps` (exported from vccs public API): `{ label, payload, coordinate, active, accessibilityLayer, ...rest }`; Tooltip also exposes `#cursor` and `#default` typed slots
