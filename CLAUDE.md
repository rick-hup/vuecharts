...
│   │   └── ui/chart/       # ChartContainer.vue (wraps ResponsiveContainer + vccs SVG theme CSS selectors), index.ts
...
**Playground stack**: Nuxt 3, Tailwind CSS v4, shadcn-nuxt, lucide-vue-next, vccs (`workspace:*`). `ChartContainer` wraps `<ResponsiveContainer>` with vccs SVG theme CSS selectors (same pattern as docs site). Pages: `index.vue` (card-grid nav with shadcn Cards + lucide icons), `bar-charts.vue` (10 variants: basic, horizontal, multiple, stacked+legend, label, custom-label, mixed per-bar fills, active hover, negative values, interactive toggle). Bar chart data: 6-month visitor data (Jan-Jun) + 90-day date series for interactive chart. State: `activeChart` ref (desktop/mobile toggle in BarChartInteractive). Notable patterns: `BarChartActive` uses `fill-opacity` + `active-fill-opacity` Bar props (no ref needed) for hover opacity effect; `BarChartMixed` passes `fill` in each data entry (no fill prop on Bar — reads from `data.fill`); `BarChartNegative` uses Vue `#shape` slot in SFC template syntax to conditionally apply fill based on value sign (`props.value >= 0 ? 'var(--chart-1)' : 'var(--chart-2)'`); `BarChartCustomLabel` uses Bar `:label="{ position: 'insideLeft', offset: 8, fill: 'white', fontSize: 12 }"` for inside-bar labels.
