...
│   └── pages/              # index.vue (card-grid nav), bar-charts.vue (renders all 10 Bar variants), area.vue
└── nuxt.config.ts          # compatibilityVersion 4, Tailwind v4 (@tailwindcss/vite), shadcn-nuxt (prefix: '', componentDir: ./app/components/ui)
```

**Playground stack**: Nuxt 3, Tailwind CSS v4, shadcn-nuxt, lucide-vue-next, vccs (`workspace:*`). `ChartContainer` accepts `config: ChartConfig` prop, provides `--color-{key}` CSS vars from config entries, injects config via `provide('chart-config')`, and wraps `<ResponsiveContainer>` with vccs SVG theme CSS selectors. `ChartConfig = Record<string, { label?, icon?, color?, theme? }>`. `ChartTooltipContent` and `ChartLegendContent` inject `chart-config` to resolve label/color per payload item. All bar chart demos use the shadcn/ui chart pattern: define `chartConfig`, pass `(props) => h(ChartTooltipContent, { ...props, [options] })` as `:content` to `<Tooltip>`, use `var(--color-{key})` for fills. Pages: `index.vue` (card-grid nav with shadcn Cards + lucide icons), `bar-charts.vue` (10 variants: basic, horizontal, multiple, stacked+legend, label, custom-label, mixed per-bar fills, active hover, negative values, interactive toggle). Bar chart data: 6-month visitor data (Jan-Jun) + 90-day date series for interactive chart. State: `activeChart` ref (desktop/mobile toggle in BarChartInteractive). Notable patterns: `BarChartActive` uses `#shape` slot with `props.isActive` for fill-opacity toggling per-bar; `BarChartMixed` is layout=vertical (horizontal bars), uses `#shape` slot reading `props.payload?.fill` (fill per data entry, no fill prop on Bar); `BarChartNegative` uses `#shape` slot with `<rect>`, y-corrected as `props.height < 0 ? props.y + props.height : props.y`, fill toggled by `props.value > 0 ? 'var(--chart-1)' : 'var(--chart-2)'`; `BarChartCustomLabel` is layout=vertical with both axes hidden, uses Bar `:label="{ dataKey: 'month', position: 'insideLeft', offset: 8, fill: 'white', fontSize: 12 }"`; `BarChartStacked` uses `ChartLegendContent` via `<Legend #content="legendProps"><ChartLegendContent v-bind="legendProps" /></Legend>`; `ChartTooltipContent` indicator prop: `'dot'` (default), `'line'`, `'dashed'`.
