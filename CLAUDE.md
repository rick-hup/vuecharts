...
│   ├── layouts/
│   │   └── default.vue     # Persistent layout: sticky header (BarChart3 icon + "vccs playground" logo, nav links /bar-charts + /area + /line-charts with active bg-accent state, dark mode toggle Moon/Sun via useColorMode()), main <slot />
│   ├── app.vue             # Root: <NuxtLayout> wrapping <NuxtRouteAnnouncer /> + <NuxtPage />
│   └── pages/              # index.vue (card-grid nav: 3 routes — bar-charts/area/line-charts, each count:10, shadcn Cards + lucide icons BarChart3/AreaChartIcon/LineChartIcon + ArrowRight "View demos" link), bar-charts.vue (10 variants: BarChartInteractive spans full md:col-span-2 lg:col-span-3, 9 others in responsive grid), area.vue (10 area chart SFC components: AreaChartInteractive spans full md:col-span-2 lg:col-span-3, 9 others in md:grid-cols-2 lg:grid-cols-3), line-charts.vue (10 variants: LineChartInteractive spans full md:col-span-2 lg:col-span-3, 9 others in md:grid-cols-2 lg:grid-cols-3)
