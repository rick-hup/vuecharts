...
### Scatter Component Pattern
- `Scatter` graphical component: props `xAxisId`/`yAxisId`/`zAxisId` (all default `0`), `dataKey`, `data` (component-level override), `name`, `hide`, `fill`, `shape` (`SymbolType`, default `'circle'`), `isAnimationActive`, `legendType`, `tooltipType`
- Registers with Redux via `useSetupGraphicalItem(props, 'scatter')`; `useScatter(props)` returns `{ shouldRender, points }`
- `shouldRender`: `!props.hide && points != null && points.length > 0`
- Renders one `Symbols` element per point (skips points with `cx == null || cy == null`); `Teleport`s into `GraphicalLayerContext` ref; CSS class `.v-charts-scatter`
- `selectScatterPoints` selector (`state/selectors/scatterSelectors.ts`) — `computeScatterPoints({ displayedData, xAxis, yAxis, zAxis, scatterSettings, xAxisTicks, yAxisTicks })`:
  - Returns `ScatterPointItem[]` with `cx`/`cy` (pixel coords), `x`/`y`/`width`/`height` (bounding box), `size`, `node {x,y,z}`, `tooltipPayload` (x+y+optional-z entries), `tooltipPosition`, `payload`
  - Data priority: `scatterSettings.data` (component-level) over chart Redux `chartData[startIndex..endIndex]`
  - `size` resolved via `zAxis.scale(z)`; `radius = sqrt(max(size,0)/PI)`; always calls `toRaw(rawEntry)` before D3 scale calls
  - `selectSynchronisedScatterSettings` validates scatter is registered in `graphicalItems` store before computing

### ZAxis Component Pattern
- `ZAxis` is a data-only component (renders `null`): dispatches `addZAxis`/`removeZAxis` to Redux via `watchEffect` + `onCleanup`
- Props: `zAxisId` (default `0`), `dataKey`, `type` (`'number'|'category'`), `range` (`[number, number]`), `domain`, `scale`, `name`, `unit`; defaults for `allowDuplicatedCategory`/`allowDataOverflow`/`reversed`/`includeHidden` come from `implicitZAxis` in `axisSelectors`
- Usage: add `<ZAxis type="number" dataKey="size" range={[0, 250]} />` inside a `ComposedChart` alongside `<Scatter>` to drive symbol sizing

### Symbols Shape Pattern
- `Symbols` functional component (not `defineComponent`) renders a D3-based SVG `<path>` via `victory-vendor/d3-shape` symbol factories
- `SymbolType`: `'circle' | 'cross' | 'diamond' | 'square' | 'star' | 'triangle' | 'wye'`; `getSymbolFactory(type)` maps to D3 factory, falls back to `symbolCircle`
- Props: `type` (default `'circle'`), `size` (default `64`), `sizeType` (`'area'|'diameter'`, default `'area'`); `cx`/`cy` required for render; remaining attrs spread onto `<path>`
- `calculateAreaSize(size, sizeType, type)` converts diameter-based size to area for each shape type
- Renders `<path transform="translate(cx, cy)" d={path} class="v-charts-symbols" />`; returns `null` when `cx`/`cy`/`size` are not numbers
- Exported from `@/shape/index.ts`

### Bar Shape Slot Pattern
- `Bar` accepts a `shape` **named slot** for custom bar rendering: `<Bar>{{ shape: (props: any) => <MyShape {...props} /> }}</Bar>`
- Slot function receives full `barRectangleProps`: position (`x`, `y`, `width`, `height`), `value`, `index`, `dataKey`, active state, and all SVG attrs
- In `BarRectangles`, slot is accessed as `shapeSlot` from `useBarContext()`: `shapeSlot ? shapeSlot(barRectangleProps) : <Rectangle {...barRectangleProps} />`
- `Bar.tsx` passes `slots.shape` into `useBar(props, slots.shape)` which stores it on `BarContext`
- `Bar` slots type: `{ default?: () => any; activeDot?: (props: any) => any; shape?: (props: any) => any }`
- Custom shape components should use `inheritAttrs: false` and read geometry from `attrs`
- Replaces the old `shape` prop pattern (VNode prop); slots are the Vue-idiomatic alternative
...
### Axis Prop Defaults
- `XAxis.dataKey` and `YAxis.dataKey` default to `undefined` (not `''`); an empty string would be treated as a valid dataKey and cause incorrect axis behavior when no dataKey is specified
- `YAxis.unit` defaults to `undefined` (not `''`); same reason — empty string is a valid unit string
- `selectHasBar(state)` uses `state.graphicalItems.cartesianItems.some(item => item.type === 'bar')` (not `countOfBars`) — the `countOfBars` field was incorrect for `ComposedChart`; always iterate `cartesianItems` to detect bar presence
...
- **Scatter component**: New `cartesian/scatter/` module; `Scatter` registers via `useSetupGraphicalItem` as type `'scatter'`; `useScatter` hook reads `selectScatterPoints` from Redux — computes `cx`/`cy` pixel positions from xAxis/yAxis scales, `size` from ZAxis scale, bounding box from radius; renders `Symbols` per point (skips null `cx`/`cy`); `Teleport`s into `GraphicalLayerContext`; supports component-level `data` prop override (takes priority over chart `chartData`); `shape` prop selects D3 symbol type (default `'circle'`)
- **ZAxis component**: New `cartesian/z-axis/` module; data-only (renders `null`); dispatches `addZAxis`/`removeZAxis` via `watchEffect` + `onCleanup`; props `zAxisId`, `dataKey`, `type`, `range`, `domain`, `scale`, `name`, `unit`; `implicitZAxis` defaults from `axisSelectors`; pairs with `Scatter` to drive symbol sizing
- **Symbols shape**: New `shape/Symbols.tsx`; functional component using `victory-vendor/d3-shape` factories; `SymbolType` union of seven D3 symbols; `calculateAreaSize` converts diameter to area per shape geometry; renders `<path transform="translate(cx,cy)" class="v-charts-symbols" />`; exported from `@/shape/index.ts`
- **Storybook ComposedChart**: `AreaChartWithAccessibilityLayer` story demonstrates keyboard accessibility — tab to focus the chart, then use arrow keys to navigate along it; references Recharts issue [#5477](https://github.com/recharts/recharts/issues/5477); uses `isAnimationActive={false}` on `Area` for deterministic rendering; `BoxPlotChart` story (`chart/__stories__/BoxPlot.stories.tsx`, `title: 'examples/ComposedChart'`) demonstrates a box plot built from stacked `Bar`s in `ComposedChart` — uses `Bar` `shape` **slot** with custom `defineComponent` shapes: `<Bar>{{ shape: (props: any) => <HorizonBar {...props} /> }}</Bar>` — `HorizonBar` (renders an `<line>` across the bar width for whisker endpoints), `DotBar` (renders a vertical dashed `<line>` for whiskers), invisible bars (`fill="none"`) for spacing; average dot overlay uses `<Scatter dataKey="average" fill="red" stroke="#FFF" />` + `<ZAxis type="number" dataKey="size" range={[0, 250]} />` (the prior `AverageDots` manual component that used `selectAxisScale`/`selectChartDataWithIndexes`/`useLabelLayerRef`+`Teleport` was removed once `Scatter`+`ZAxis` were ported); `boxPlotData` entries carry `size: 250` for ZAxis sizing; custom shape components use `inheritAttrs: false` and read `x`/`y`/`width`/`height` from `attrs`; data fields: stacked (`min`, `bottomWhisker`, `bottomBox`, `topBox`, `topWhisker`) + shape-slot whisker bars (`bar-min`, `bar-avg`, `bar-max`) + non-stacked `average` + `size`
