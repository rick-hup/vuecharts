...
- **Bar component**: Composes BarBackground + BarRectangles directly (RenderBar removed); clipping via useNeedsClip + GraphicalItemClipPath; `activeBar` prop on BarRectangles gates highlight (`false` disables, object merges extra SVG props onto active rect); provides `ErrorBarContext` to child slots; `label` prop (boolean or object) renders `LabelList` after `BarRectangles` — when object, its keys are spread as props onto `LabelList` and `data={barData.value}` is always passed; `background` prop now accepts `boolean | Record<string, any>` — when an object, its keys are merged into `barRectangleProps` with highest priority (after `backgroundFromDataEntry`) inside `BarBackground`, allowing per-bar background SVG prop overrides; `BarRectangleItem` carries `tooltipPosition` (center of bar) and `stackedBarStart: number` (pixel coordinate of the stack base value, from `numericAxis.scale(baseValue)` in `computeBarRectangles`, defaults to 0); animation uses an incrementing `animationId` integer as the `Animate` key — increments on every data change, causing `Animate` to remount and restart its 0→1 animation; `previousRectangles` is updated at `t > 0` (inside the animation tick, not just at `t === 1`) so rapid restarts during Brush drag interpolate from the current visual position rather than the original start, producing a smooth "chase" effect; new bars (no previous data) animate from `stackedBarStart` — horizontal: `y` interpolates from `stackedBarStart` → `entry.y`, `height` from `0` → `entry.height`; vertical: `x` interpolates from `stackedBarStart` → `entry.x`, `width` from `0` → `entry.width`
...
- **Storybook BarChart**: Stories cover `StackedAndDynamic` (interactive legend-driven series toggling), `StackedWithErrorBar` (vertical layout, `direction="x"` error bars), `XAxisTickMarginWithBrushDy` (XAxis `tickMargin` + Brush `dy` + panorama mini-chart as Brush slot child), `StackedWithBrush` (stacked bars with basic Brush integration, no `dy` offset or Panorama), `HasLabelBasedOnSeparateDataKey` (Bar `label` prop with `dataKey` pointing to a derived field, data enriched inside `render`), and `NoPadding` (`Bar` `background` as object `{ fill: '#eee' }`, `XAxis scale="point"` with `padding`, fixed `barSize`)
