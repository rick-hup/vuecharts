...
- `Pie` `dataKey` is required; `nameKey` default `'name'`; `cx`/`cy` default to `'50%'`; `outerRadius` default `'80%'`; `innerRadius` default `0` (solid pie); `startAngle`/`endAngle` default `0`/`360`; `paddingAngle`/`minAngle` default `0`; `fill` default `'#808080'`; `stroke` default `'#fff'`; `legendType` default `'rect'`; data-item `fill` overrides component `fill`; requires `SetPolarGraphicalItem` to dispatch before `selectPieSectors` returns data (`selectSynchronisedPieSettings` gates on matching polar item with same `dataKey`); `parseCoordinateOfPie` offset math: `cx = offset.left + getPercentValue(cx%, offset.width, width/2)`; `label={true}` renders a `<g>` per sector containing: a `<line>` from `outerRadius` edge to label position (stroke=`sector.fill`, fill=none) + a `<text>` at `outerRadius + 20` (`LABEL_OFFSET=20`) via `polarToCartesian(cx, cy, outerRadius+20, midAngle)`; text-anchor: `'start'` if `pos.x > cx`, `'end'` if `pos.x < cx`, `'middle'` otherwise; label text is `String(sector.value)` colored with `sector.fill`; CSS class: `v-charts-pie` (Layer); `shape` **named slot** for custom sector rendering: `<Pie>{{ shape: (props) => <MySector {...props} /> }}</Pie>` — slot receives `PieSectorDataItem & { isActive: boolean }`; wraps each sector in a `<g>` with `onMouseenter`/`onMouseleave` to set `activeIndex`; `isActive` is `true` for the hovered sector, enabling the active-shape expand pattern (see `CustomActiveShapePieChart` story); `activeIndex` prop (default `-1`) initializes the active sector — set to `0` to replicate Recharts `Tooltip defaultIndex={0}` behavior; when `slots.shape` is absent, default `<Sector>` is rendered with hover handlers directly; `isAnimationActive` prop: uses `<Animate from={0} to={1}>` to animate each sector's `endAngle` from `startAngle` to full sweep; labels rendered only after `progress >= 1`; label slot not yet implemented
...
- **PieChart + Pie + DraggablePie** (`polar/pie/`, `shape/Sector.tsx`, `chart/PieChart.ts`): `computePieSectors` pure function in `state/selectors/pieSelectors.ts` (exports `PieSectorDataItem`, `ResolvedPieSettings`, `selectPieSectors`); `SetPolarGraphicalItem` in `state/SetGraphicalItem.ts` mirrors `SetCartesianGraphicalItem` pattern + `replacePolarGraphicalItem` reducer in `graphicalItemsSlice.ts`; `getRelativeCoordinate` utility (`utils/getRelativeCoordinate.ts`) for mouse-relative coords (see Mouse Coordinate Mapping pattern); story at `chart/__stories__/DraggablePie.stories.tsx` (`title: 'Examples/Pie/DraggablePie'`); `computeAngle` formula: `-Math.atan2(dy,dx)*(180/PI)` clamped `[0,360)`; draggable `<circle>` placed after `<Pie>` in SVG slot (SVG paint-order z-ordering, no ZIndexLayer needed); `label={true}` renders `<text>` value labels at `outerRadius + 20` (LABEL_OFFSET=20), text-anchor based on `pos.x` vs `cx`; `shape` slot implemented (see Key Component Gotchas); entrance animation via `<Animate from={0} to={1}>` now implemented; label slot not yet implemented; **CustomActiveShapePieChart story** (`chart/__stories__/CustomActiveShapePieChart.stories.tsx`, `title: 'Examples/Pie/CustomActiveShapePieChart'`): demonstrates `shape` named slot — `renderActiveShape(sector)` checks `isActive` to conditionally render expanded sector (outer ring `<Sector>` at `outerRadius+6`→`outerRadius+10`, connector path `M${sx},${sy}L${mx},${my}L${ex},${ey}`, value + percent labels) vs plain `<Sector>`; uses `innerRadius={60} outerRadius={80}` (donut)
<!-- END AUTO-MANAGED -->
