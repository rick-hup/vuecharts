...
- `DocsToc`: sticky TOC sidebar (`hidden xl:block w-44`); props `links` (TocLink[] from `page.body.toc.links`); active heading via `useToc().activeId` (IntersectionObserver on `.prose h2[id], .prose h3[id]` with `rootMargin: '-80px 0px -80% 0px'`); depth-3 links indented (`pl-3`); active link styled `!text-primary font-medium`
- `docs/[...slug].vue`: catch-all `layout: docs`, `queryCollection(collectionName).path(slug).first()`; slug `/docs/<path>` from `route.params.slug` (path = `params.slug` joined by `/`); `useAsyncData` with `watch: [slug]`; TOC from `page.body.toc.links`, prev/next from flattened `queryCollectionNavigation`; renders `ContentRenderer` + `DocsPagination` + `DocsToc`
