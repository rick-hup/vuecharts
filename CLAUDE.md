...
├── app/
│   ├── components/
│   │   ├── area-charts/    # 10 Area chart SFCs (auto-imported as AreaChartsAreaChart*): AreaChartDefault, AreaChartLinear, AreaChartStep, AreaChartStacked, AreaChartStackedExpand, AreaChartGradient, AreaChartLegend, AreaChartIcons, AreaChartInteractive, AreaChartAxes
...
│   │   ├── line-charts/    # 10 Line chart SFCs (auto-imported as LineChartsLineChart*): LineChartDefault (type="natural"), LineChartLinear (type="linear"), LineChartStep (type="step"), LineChartMultiple (desktop+mobile, type="monotone"), LineChartDots (:dot object + :active-dot), LineChartDotsColors (per-entry fill + YAxis), LineChartLabel (LabelList child of Line), LineChartCustomLabel (two Lines with top/bottom LabelList, indicator="line"), LineChartLegend (Legend+ChartLegendContent slot), LineChartInteractive (time-range Select 90d/30d/7d, filteredData computed, labelFormatter for dates)
│   │   ├── radar-charts/   # 11 Radar chart SFCs (auto-imported as RadarChartsRadarChart*): RadarChartDefault, RadarChartDots, RadarChartMultiple, RadarChartLinesOnly, RadarChartGridCircle, RadarChartGridCircleFill, RadarChartGridFill, RadarChartGridNone, RadarChartCustomLabel, RadarChartLegend, RadarChartRadius
│   │   ├── radial-charts/  # 6 RadialBar chart SFCs (auto-imported as RadialChartsRadialChart*): RadialChartSimple, RadialChartGrid, RadialChartLabel, RadialChartShape, RadialChartStacked, RadialChartText
│   │   ├── tooltip-charts/ # 9 Tooltip demo SFCs (auto-imported as TooltipChartsTooltip*): TooltipDefault, TooltipLineIndicator, TooltipNoIndicator, TooltipCustomLabel, TooltipLabelFormatter, TooltipNoLabel, TooltipFormatter, TooltipIcons, TooltipAdvanced
...
│   ├── layouts/
│   │   └── default.vue     # Persistent layout: sticky header (BarChart3 icon + "vccs playground" logo, nav links /bar-charts + /area + /line-charts + /pie-charts + /radar-charts + /radial-charts + /tooltip-charts with active bg-accent state, dark mode toggle Moon/Sun via useColorMode()), main <slot />
│   ├── app.vue             # Root: <NuxtLayout> wrapping <NuxtRouteAnnouncer /> + <NuxtPage />
│   └── pages/              # index.vue (card-grid nav: 7 routes — bar-charts/area/line-charts/pie-charts/radar-charts/radial-charts/tooltip-charts, counts: 10/10/10/10/11/6/9, grid grid-cols-1/sm:grid-cols-2/lg:grid-cols-4, shadcn Cards + lucide icons + ArrowRight "View demos" link), bar-charts.vue (10 variants: BarChartInteractive spans full md:col-span-2 lg:col-span-3, 9 others in responsive grid), area.vue (10 area chart SFC components: AreaChartInteractive spans full md:col-span-2 lg:col-span-3, 9 others in md:grid-cols-2 lg:grid-cols-3), line-charts.vue (10 variants: LineChartInteractive spans full md:col-span-2 lg:col-span-3, 9 others in md:grid-cols-2 lg:grid-cols-3), radar-charts.vue (11 variants in grid-cols-1/md:grid-cols-2/lg:grid-cols-3, no full-width interactive item), radial-charts.vue (6 variants in grid-cols-1/md:grid-cols-2/lg:grid-cols-3: Simple, Grid, Label, Shape, Stacked, Text), tooltip-charts.vue (9 variants in grid-cols-1/md:grid-cols-2/lg:grid-cols-3: Default, LineIndicator, NoIndicator, CustomLabel, LabelFormatter, NoLabel, Formatter, Icons, Advanced)
...
**Playground stack**: Nuxt 3, Tailwind CSS v4, shadcn-nuxt, lucide-vue-next, vccs (`workspace:*`). `playground/nuxt/app/assets/css/tailwind.css`: @import tailwindcss + tw-animate-css, `@custom-variant dark (&:is(.dark *))`, @theme inline (shadcn color vars + chart-1..5 + sidebar variants), `:root`/`.dark` oklch color tokens, `@utility container` (responsive padding-inline), `@layer base` (`*` border-border outline-ring/50, body bg-background text-foreground). No font utilities, no `.prose`, no `.no-scroll-bar`, no callout vars, no Shiki theme vars — those are docs-site only. `ChartContainer` accepts `config: ChartConfig` prop, provides `--color-{key}` CSS vars from config entries, injects config via `provide('chart-config')`, and wraps `<ResponsiveContainer>` with vccs SVG theme CSS selectors. `ChartConfig = Record<string, { label?, icon?, color?, theme? }>`. `ChartTooltipContent` and `ChartLegendContent` inject `chart-config` to resolve label/color per payload item. **`ChartTooltipContent` props**: `active`, `payload: ReadonlyArray<...>`, `label`, `labelFormatter: (value, payload: ReadonlyArray<...>) => string`, `nameKey`, `indicator` (`'dot'`(default)|`'line'`|`'dashed'`), `hideLabel`, `hideIndicator`, `labelKey`, `className`. **`getPayloadConfig(item)` resolution order**: (1) `cfg[nameKey]` or `cfg[payload[nameKey]]` if `nameKey` provided, (2) `cfg[dataKey]`, (3) fallback to `item.name`/`item.color`; returns `{ label?, color?, icon? }` — `icon` sourced from ChartConfig entry. **`formattedLabel`**: applies `labelFormatter` if provided, then `cfg[labelKey].label` if `labelKey` set, then raw `label`; label header is suppressed when `indicator === 'line'` (only shown for `'dot'`/`'dashed'`). **`nestLabel`**: `true` when `payload.length === 1 && indicator !== 'dot'` — renders label nested inside the payload row instead of as a header. **Indicator rendering**: if ChartConfig entry has an `icon` field, renders `<component :is="icon" />` instead of the colored shape; otherwise renders the shape div with CSS vars `--color-bg`/`--color-border` via **Tailwind v4 parenthesis syntax**: `border-(--color-border) bg-(--color-bg)` (NOT square-bracket `border-[--color-border]` — that is v3 syntax); dot=`h-2.5 w-2.5`, line=`w-1`, dashed=`border-dashed bg-transparent`. Values formatted with `.toLocaleString()` if number. All bar, area, line, and radar chart demos use the shadcn/ui chart pattern: define `chartConfig`, use `<Tooltip><template #content="tooltipProps"><ChartTooltipContent v-bind="tooltipProps" [options] /></template></Tooltip>` (preferred slot pattern), use `var(--color-{key})` for fills. Pages: `index.vue` (card-grid nav with shadcn Cards + lucide icons, 7 routes: bar-charts/area/line-charts/pie-charts/radar-charts/radial-charts/tooltip-charts), `bar-charts.vue` (10 variants: basic, horizontal, multiple, stacked+legend, label, custom-label, mixed per-bar fills, active hover, negative values, interactive toggle), `area.vue` (10 variants: default natural curve, linear, step, stacked `stack-id="a"`, stacked-expand `stack-offset="expand"`, gradient `fill="url(#fillKey)"` with `<linearGradient>` defs inside `<AreaChart>`, legend+`ChartLegendContent`, icons with `icon` field in chartConfig entries, interactive with date-filtered 90-day data + time range Select + gradient fills, axes with `<YAxis>`), `line-charts.vue` (10 variants: default, linear, step, multiple, dots, dots-colors, label, custom-label, legend, interactive), `radar-charts.vue` (11 variants: default, dots, multiple, lines-only, grid-circle, grid-circle-fill, grid-fill, grid-none, custom-label, legend, radius-axis), `radial-charts.vue` (6 variants: simple, grid, label, shape, stacked, text). Area chart data: 6-month visitor data (Jan-Jun) for most variants; 90-day date series (2024-04-01 → 2024-06-30, desktop+mobile) for interactive chart. Notable area patterns: `AreaChartStackedExpand` uses `stack-offset="expand"` on `<AreaChart>` for 100% stacked; `AreaChartGradient` defines `<linearGradient>` as `<defs>` child inside `<AreaChart>` template; `AreaChartIcons` sets `icon` (lucide component) on chartConfig entries — displayed by `ChartLegendContent`; `AreaChartAxes` adds `<YAxis :tick-line="false" :axis-line="false" :tick-count="3">`; `AreaChartLinear` uses `hide-label` on `ChartTooltipContent`; `AreaChartStep` uses `type="step"` on `<Area>` with `hide-label` tooltip. Bar chart data: 6-month visitor data (Jan-Jun) + 90-day date series for interactive chart. State: `activeChart` ref (desktop/mobile toggle in BarChartInteractive). Notable bar patterns: `BarChartActive` uses `:active-index="2"` to pin a bar as permanently active (independent of tooltip hover), `#shape` slot for base per-bar rendering (`<Rectangle v-bind="props" :fill="props.payload?.fill" />`), and `#activeBar` slot for the highlighted state (`fill-opacity=0.8`, `stroke-dasharray=4`, `stroke-dashoffset=4`, stroke matching entry fill); `BarChartMixed` is layout=vertical (horizontal bars), uses `#shape` slot reading `props.payload?.fill` (fill per data entry, no fill prop on Bar), uses `#content` slot on Tooltip with `hide-label` (migrated from `:content` render-function prop); `BarChartNegative` uses `<Cell v-for="(item, index) in data" :key="index" :fill="item.visitors > 0 ? 'var(--chart-1)' : 'var(--chart-2)'">` as children of `<Bar>` to color each bar by sign (positive=`--chart-1`, negative=`--chart-2`); `<LabelList position="top" data-key="month" :fill-opacity="1">` as sibling child of `<Bar>`; imports `Cell, LabelList` from `vccs`; tooltip uses `#content` slot with `hide-label` + `hide-indicator`; no XAxis; chartConfig has no color entry (colors driven entirely by Cell fill, not `--color-{key}` vars); `BarChartLabel` uses `<LabelList position="top" :offset="12" class="fill-foreground" :font-size="12" />` as a child slot of `<Bar>` (no `data-key` needed — inherits from Bar), `:margin="{ top: 20 }"` on BarChart to provide space above bars; tooltip uses `#content` slot with `hide-label`; imports `LabelList` from `vccs`; `BarChartCustomLabel` is layout=vertical with both axes hidden (`:hide="true"`), chartConfig must include `label: { color: 'var(--background)' }` key; uses two `<LabelList>` children inside `<Bar>`: (1) `data-key="month"` `position="insideLeft"` `class="fill-(--color-label)"` `:font-size="12"` and (2) `data-key="desktop"` `position="right"` `class="fill-foreground"` `:font-size="12"`; tooltip uses `#content` slot (not `:content` prop); imports `LabelList` from `vccs`; `BarChartInteractive` chartConfig: desktop=`'var(--chart-2)'`, mobile=`'var(--chart-1)'`; uses `#content` slot pattern (`<Tooltip><template #content="tooltipProps"><ChartTooltipContent v-bind="tooltipProps" name-key="views" :label-formatter="labelFormatter" class="w-[150px]" /></template></Tooltip>`) — not the `:content` prop with `h()`; `labelFormatter` formats date string for display; `BarChartStacked` uses `ChartLegendContent` via `<Legend #content="legendProps"><ChartLegendContent v-bind="legendProps" /></Legend>`. Line chart data: 6-month visitor data (Jan-Jun, desktop only for most variants; desktop+mobile for multiple/custom-label/legend/interactive); browser-based dataset (Chrome/Safari/Firefox/Edge/Other visitors) for DotsColors; 90-day date series (2024-04-01 → 2024-06-30, desktop+mobile) for interactive. Notable line patterns: `LineChartDots` uses `:dot="{ fill: 'var(--color-desktop)' }"` to show colored dots on every point, `:active-dot="{ r: 6 }"` for hover size; `LineChartDotsColors` uses `#dot` slot on `<Line>` to render `<Dot :fill="dotProps.payload?.fill" :stroke="dotProps.payload?.fill">` — fill sourced from `payload.fill` field in data entries (set to `var(--color-{browser})`); chartConfig defines per-browser color keys; line stroke is uniform (`var(--color-visitors)`); XAxis only (no YAxis); `LineChartLabel`/`LineChartCustomLabel` place `<LabelList>` as a **child slot inside `<Line>`** (not at chart level) — `position="top"` or `"bottom"`, `:offset="12"`, `class="fill-foreground"`, `:font-size="12"`; `LineChartCustomLabel` also uses `indicator="line"` on `ChartTooltipContent` and `:margin="{ left: 12, right: 12, top: 20, bottom: 10 }"`; `LineChartLegend` and `LineChartInteractive` use the same `<Legend #content="legendProps"><ChartLegendContent v-bind="legendProps" /></Legend>` pattern as `BarChartStacked`; `LineChartInteractive` uses the same time-range Select (90d/30d/7d) + `filteredData` computed pattern as `AreaChartInteractive`, with `labelFormatter` for date display. Radar chart data: 6-month visitor data (Jan-Jun, desktop only for most; desktop+mobile for multiple/lines-only/legend/radius). Notable radar patterns: `RadarChartDefault` uses `<PolarAngleAxis data-key="month" />` + `<PolarGrid />` + `<Radar data-key="desktop" fill="var(--color-desktop)" :fill-opacity="0.6" />`; `RadarChartDots` adds `:dot="{ r: 4, fillOpacity: 1 }"`; `RadarChartGridCircle` uses `<PolarGrid grid-type="circle" />` with hide-label tooltip; `RadarChartGridCircleFill` applies `class="fill-(--color-desktop) opacity-20"` on `<PolarGrid grid-type="circle">` to fill circular grid with chart color; `RadarChartGridFill` same class on default polygon grid; `RadarChartGridNone` omits `<PolarGrid>` entirely; `RadarChartLinesOnly` uses two `<Radar>` series with `:fill-opacity="0"` + explicit `stroke` + `:stroke-width="2"`, `<PolarGrid :radial-lines="false" />`, `indicator="line"` tooltip; `RadarChartMultiple` has two `<Radar>` series (desktop+mobile) + `indicator="line"` tooltip; `RadarChartCustomLabel` uses the `#tick` slot on `<PolarAngleAxis>` to render custom SVG `<text>` with two `<tspan>` lines — desktop/mobile values (`fill-foreground`, `dy="-0.6em"`) above and abbreviated month name (`fill-muted-foreground`, `dy="1.2em"`) below; slot props: `{ x, y, value, textAnchor }`; uses two `<Radar>` series (desktop `:fill-opacity="0.6"`, mobile no fill-opacity) and `indicator="line"` tooltip; `RadarChartLegend` uses `<Legend #content="legendProps"><ChartLegendContent v-bind="legendProps" /></Legend>` + `:margin="{ top: -40, bottom: -10 }"`; `RadarChartRadius` adds `<PolarRadiusAxis :angle="60" orientation="middle" :axis-line="false" stroke="hsla(var(--foreground))" />` + `label-key="month"` on tooltip. Radial chart data: browser visit data (5 entries: chrome/safari/firefox/edge/other, each with `fill: 'var(--color-{browser})'`) for most demos; single-entry data for shape/text/stacked variants. Notable radial patterns: all radial chart containers use `class="mx-auto aspect-square max-h-[250px]"` (NOT `aspect-video`); `RadialChartSimple` uses `<RadialBarChart :inner-radius="30" :outer-radius="110">` + `<RadialBar data-key="visitors" :background="true" />` + tooltip `name-key="browser" hide-label`; `RadialChartGrid` adds `<PolarGrid grid-type="circle" />`; `RadialChartLabel` uses `start-angle="-90" end-angle="380"` + `<LabelList position="insideStart" data-key="browser" class="fill-white capitalize mix-blend-luminosity" :font-size="11" />` as child slot inside `<RadialBar>`; `RadialChartShape` uses single-entry data, `end-angle="100" inner-radius="80" outer-radius="140"`, `<PolarGrid grid-type="circle" :radial-lines="false" stroke="none" class="first:fill-muted last:fill-background" :polar-radius="[86, 74]" />`; `RadialChartText` same PolarGrid pattern with `start-angle="0" end-angle="250" inner-radius="80" outer-radius="110"`; `RadialChartStacked` uses single entry `{ desktop: 1260, mobile: 570 }`, `end-angle="180" inner-radius="80" outer-radius="130"`, two `<RadialBar stack-id="a" :corner-radius="5" class="stroke-transparent stroke-2">`. **Label-inside-PolarRadiusAxis pattern** (shape/text/stacked): `<PolarRadiusAxis :tick="false" :tick-line="false" :axis-line="false"><Label><template #content="{ viewBox }">...</template></Label></PolarRadiusAxis>` — `viewBox` provides `{ cx, cy }` via `POLAR_LABEL_VIEW_BOX_KEY` context (provided by `PolarRadiusAxis` to its slot children); render two `<tspan>` (value at `cy` and label at `cy + 24`) inside a `<text dominant-baseline="middle">`. Tooltip chart data: date-based activity data (2024-07-15 to 2024-07-20, running+swimming kcal values), two stacked `<Bar stack-id="a">` in `<BarChart>`, `<XAxis :tick-formatter>` formatting date to weekday short, `<Tooltip :cursor="false" :default-index="1">`. Notable tooltip patterns: `TooltipDefault` — `<ChartTooltipContent>` with no extra props (default dot indicator + label header); `TooltipLineIndicator` — `indicator="line"`; `TooltipNoIndicator` — `hide-indicator`; `TooltipNoLabel` — `hide-indicator hide-label`; `TooltipCustomLabel` — `label-key="activities" indicator="line"` (chartConfig must include `activities: { label: 'Activities' }` key; `label-key` picks the label from config); `TooltipLabelFormatter` — `:label-formatter="labelFormatter"` (date → "July 15, 2024" with `toLocaleDateString`); `TooltipIcons` — `hide-label` + chartConfig entries with `icon: Footprints/Waves` (icons replace color indicators); `TooltipFormatter` — fully custom `#content` slot (no ChartTooltipContent), manual `v-for payload` div, CSS var colors `var(--color-{dataKey})`, unit suffix "kcal", `getLabel()` helper resolving chartConfig labels; `TooltipAdvanced` — fully custom `#content` slot, per-item rows + total row after last item (index === payload.length - 1), running+swimming sum with "kcal".
...
### SVG Layers (Teleport)
Three-tier z-ordering: cursor → graphical → label (via `Surface.vue`). `Area` and `Line` use `<Teleport to={graphicalLayerRef.value}>` (via `useGraphicalLayerRef(null)` from `context/graphicalLayerContext`) to render their content into the graphical layer, ensuring they appear above the cursor. Pattern: call `useGraphicalLayerRef(null)` in setup; wrap render output in `<Teleport>` when `graphicalLayerRef.value` is truthy, otherwise return content directly.
...
### PolarAngleAxis
- Props: `angleAxisId` (default `0`), `dataKey` (DataKey), `tick` (default `true`), `tickLine` (default `true`), `tickSize` (default `8`), `axisLine` (default `true`), `axisLineType` (`'polygon'|'circle'`, default `'polygon'`), `orientation` (`'inner'|'outer'`, default `'outer'`), `tickFormatter`, `ticks` (AxisTick[]), `stroke`, `type` (`'category'|'number'`, default `'category'`), `domain`, `tickCount`
- Dispatches `addAngleAxis`/`removeAngleAxis` via `watchEffect`/`onUnmounted`
- Named slot: `#tick` with props `{ x, y, value, index, textAnchor, payload, cx, cy }` — override default `<Text>` tick label with custom SVG
- Full-circle dedup: when `|endAngle - startAngle| === 360`, removes the last tick if it coincides with the first (prevents duplicate tick at 360°=0°)
- `textAnchor` computed by `cos(-angle)` sign (start/end/middle); `verticalAnchor` by `cos`/`sin` at 45° boundary
- CSS classes: `v-charts-polar-angle-axis` (outer g), `v-charts-polar-angle-axis-ticks` (tick group g), `v-charts-polar-angle-axis-tick` (per-tick g), `v-charts-polar-angle-axis-tick-line` (line), `v-charts-polar-angle-axis-tick-value` (Text)
- Axis line: `axisLineType='polygon'` → `<polygon>` connecting tick points; `axisLineType='circle'` → `<circle>` at outerRadius

### PolarGrid
- Props: `angleAxisId` (default `0`), `radiusAxisId` (default `0`), `gridType` (`'polygon'|'circle'`, default `'polygon'`), `radialLines` (default `true`), `stroke` (default `'#ccc'`), `fill` (default `'none'`), `polarRadius` (number[] override), `strokeWidth`
- Reads `selectPolarGridAngles` + `selectPolarGridRadii` from Redux; `polarRadius` prop overrides Redux-derived radii
- Background fill: when `fill` is not `'none'`, renders a single shape at `Math.max(...radii)` first (behind concentric rings)
- `radialLines=false`: suppresses the radial spoke lines (useful for `RadarChartLinesOnly` where only concentric rings are shown)
- CSS classes: `v-charts-polar-grid` (outer g), `v-charts-polar-grid-concentric` (concentric shapes g), `v-charts-polar-grid-angle` (radial lines g)

### PolarRadiusAxis
- Props: `radiusAxisId` (default `0`), `dataKey` (DataKey, default `undefined` — passed through to Redux store), `angle` (default `0`), `tick` (default `true`), `axisLine` (default `true`), `orientation` (`'left'|'right'|'middle'`, default `'right'` — controls textAnchor), `tickFormatter`, `stroke` (default `'#ccc'`), `allowDecimals` (default `false`), `domain`, `tickCount` (default `5`), `type` (`'number'|'category'|'auto'`, default `'auto'`)
- `type='auto'` resolves via `resolveAxisType()` using chart layout: radial layout → `'category'`, centric layout → `'number'`; explicit `'number'`/`'category'` bypasses auto-resolution
- Default `domain` is `[0, 'auto']` (not `undefined`) — matches Recharts v2 behavior; creates extra band entries via `parseSpecifiedDomain` that halve bar thickness, which is the expected visual result
- Dispatches `addRadiusAxis`/`removeRadiusAxis`; renders `null` only when viewBox is null AND no ticks AND axisLine is false AND no slot children (renders when slots exist even with `tick=false axisLine=false`)
- Provides `POLAR_LABEL_VIEW_BOX_KEY` to slot children — allows `<Label>` inside `<PolarRadiusAxis>` to access `{ cx, cy }` polar viewBox for custom `#content` slot rendering
...
- Slots: `#default` — slot children (e.g., `<ErrorBar>`, `<LabelList>`) rendered outside the animation block when not animating (same pattern as `Line`/`Bar`)
- `background` prop: renders background `<Sector>` elements behind each data sector with `fill="#eee"` `fill-opacity=0.5`; pass object to override background sector props; `cornerRadius`/`forceCornerRadius`/`cornerIsExternal` forwarded to background sectors
...
- `provideCartesianLabelListData`: called in setup with `lineData` so child `<LabelList>` components placed in the `default` slot receive point data via context (same mechanism as `Bar` and `RadialBar`)
- Uses `<Teleport>` into graphical layer (see SVG Layers section); falls back to inline render when no graphical layer ref
- CSS class: `v-charts-line` (Layer)

### Radar
- Props: `dataKey` (required), `name`, `angleAxisId`/`radiusAxisId` (default `0`), `fill` (default `'#808080'`), `stroke`, `fillOpacity` (default `0.6`), `strokeWidth`, `strokeDasharray`, `dot` ([Boolean, Object], default `false`), `hide` (default `false`), `legendType` (default `'rect'`), `tooltipType`, `connectNulls` (default `false`), `label` (default `false`), `isAnimationActive` (default `true`), `activeDot` (default `true`)
- `getLegendItemColor`: stroke-preferring (stroke if set and not `'none'`, otherwise fill) — same as Line/Area
- Polygon path: `getSinglePolygonPath` repeats first point at end before closing with `Z` (ensures correct SVG fill for range paths)
- Range path: `getRangePath` = outer polygon (without final `Z`) + reversed inner polygon joined — used when `baseLinePoints` is present
- Stroke rendering (range mode): draws two separate stroke-only `<path>` elements (outer + baseline) so fill and stroke are independent layers
- Animation chase pattern: `prevPoints` + incrementing `animationId` as `<Animate key>`; new points animate from chart center (`entry.cx`/`entry.cy`); existing points interpolate from previous position
- Uses `<Teleport>` into graphical layer (same pattern as Area/Line via `useGraphicalLayerRef`)
- CSS classes: `v-charts-radar` (outer Layer), `v-charts-radar-polygon` (inner `<g>`)
- Registers via `SetPolarGraphicalItem` (type: `'radar'`), `SetLegendPayload`, `SetTooltipEntrySettings`
- `provideCartesianLabelListData`: provides point positions for child `<LabelList>` (suppressed while animating)
