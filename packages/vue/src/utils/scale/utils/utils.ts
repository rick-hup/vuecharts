...
function curry0(fn: (...args: any[]) => any) {
  return function _curried(...args: any[]) {
...
function curryN(n: number, fn: (...args: any[]) => any) {
  if (n === 1) {
    return fn
  }
...
export function reverse<T extends any[] | string>(arr: T): T {
  if (Array.isArray(arr)) {
...
export function memoize<F>(fn: F): F {
  ...
  return ((...args: Parameters<F extends (...args: any) => any ? F : any>) => {
    if (lastArgs && args.every((val, i) => val === lastArgs?.[i])) {
      return lastResult
    }

    lastArgs = args
    lastResult = (fn as any)(...args)
