import { isNan } from '@/utils/validate'

const MULTIPLY_OR_DIVIDE_REGEX = /(-?\d+(?:\.\d+)?[a-z%]*)([*/])(-?\d+(?:\.\d+)?[a-z%]*)/i
const ADD_OR_SUBTRACT_REGEX = /(-?\d+(?:\.\d+)?[a-z%]*)([+-])(-?\d+(?:\.\d+)?[a-z%]*)/i
const CSS_LENGTH_UNIT_REGEX = /^px|cm|vh|vw|em|rem|%|mm|in|pt|pc|ex|ch|vmin|vmax|Q$/
const NUM_SPLIT_REGEX = /(-?\d+(?:\.\d+)?)([a-z%]+)?/i
